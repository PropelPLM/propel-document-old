...
class Template {
  ...
  async start() {
    try {
      ...
      // upload to sf
      await this.insertNewDocument(this.orgId, this.recordId)

      // log
      await Database.insertRecord(this.orgId, 'template', this.newFileName, hummusUtils.getPageCount(this.tempFilPDFName))
  ...
  insertNewDocument(orgId, recordId) {
    return new Promise((resolve, reject) => {
      ...
      fs.createReadStream(this.tempFilePDFName).on('end', () => {
